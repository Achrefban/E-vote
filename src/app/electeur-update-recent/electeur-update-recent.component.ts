...
@Component({
  selector: 'app-electeur-update-recent',
  templateUrl: './electeur-update-recent.component.html',
  styleUrls: ['./electeur-update-recent.component.scss'],
  providers: [ElecteurService]
})
export class ElecteurUpdateRecentComponent implements OnInit {
  ...
  update() {
    ...
    this.electeurService.updateElecteur(this.electeurId, this.electeur).subscribe(data => {
      if (!data.errors) {
        ...
        this.router.navigate(['electeur/recent']);
      } else {
